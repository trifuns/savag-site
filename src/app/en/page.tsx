...
const services: { slug: string; title: string; desc: string; Icon: LucideIcon }[] = [
  ...
  { slug: "kodiranje-i-adaptacije", title: "Coding & adaptations", desc: "Module coding, adaptations, service resets.", Icon: SlidersHorizontal },
...
export default function HomeEn() {
  return (
    <div className="space-y-16">
      ...
      {/* SERVICES */}
      <section id="usluge">
        <SectionTitle kicker="Services" title="What we do" />
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {services.map(({ title, desc, slug, Icon }) => (
            <Link key={slug} href={`/en/usluge/${slug}`} className="block">
              <div
                key={title}
              ...
              >
                ...
                <div className="relative flex items-start gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 text-xs text-white/55">Efficiency • Quality • Tradition</div>
                  </div>
                </div>
              </div>
            </Link>
        ...
        <div className="mt-6">
          <Link href="/en/usluge" className="text-sm text-white/80 hover:text-white">
            View all services →
          </Link>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-8">
        <SectionTitle kicker="Why us" title="A premium service standard" />
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
            <div className="font-medium">Factory-level diagnostics</div>
            <p className="mt-2 text-sm text-white/70">
              Authorized access to coding, software updates and official procedures.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
            ...
            <p className="mt-2 text-sm text-white/70">
              OEM or high-quality aftermarket parts, with warranty up to 25 months.
            </p>
          </div>

          <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
            <div className="font-medium">Transparent recommendations</div>
            <p className="mt-2 text-sm text-white/70">
              Clear diagnostics and advice — no guessing and no unnecessary costs.
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold md:text-3xl">A look inside the workshop</h2>
            <p className="mt-2 max-w-2xl text-sm text-white/70">
              Space, equipment and work details — a selection of photos from everyday service.
            </p>
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-12">
          <div className="grid gap-4 lg:col-span-7">
            {[
              { src: "/galerija/01.jpg", alt: "Workshop" },
              { src: "/galerija/02.jpg", alt: "Service — detail" },
            ].map((img, i) => (
              <Link
                key={i}
                href="/en/galerija"
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  ...
                  <div className="mt-1 text-xs text-white/60">Open gallery →</div>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:col-span-5 lg:grid-cols-2">
            {[
              { src: "/galerija/03.jpg", alt: "Service" },
              { src: "/galerija/04.jpg", alt: "EV vehicles" },
              { src: "/galerija/05.jpg", alt: "Other brands" },
              { src: "/galerija/06.jpg", alt: "Diagnostics" },
            ].map((img, i) => (
              <Link
                key={i}
                href="/en/galerija"
        ...
        <div className="grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-semibold leading-tight tracking-tight md:text-3xl">
              Exciting partnership: our CEO Petar collaborates with Topdon on the “Share your story” campaign
            </h2>

            <p className="mt-4 max-w-xl text-sm leading-relaxed text-white/70">
              Auto Servis Savić is proud that 2024 was marked by a partnership with the global leader, Topdon!
            </p>

            <div className="mt-6">
              <Link
                href="/en/novosti/topdon-share-your-story"
                className="inline-flex items-center justify-center rounded-2xl border border-white/15 bg-white/10 px-6 py-3 text-sm font-medium text-white hover:bg-white/15"
              >
                Click to learn more
              </Link>
            </div>
        ...
        <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 text-white/75">
              Send an inquiry or call us — we reply quickly during business hours.
            </p>
          </div>
